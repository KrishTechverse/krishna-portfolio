...
const DotMatrix: React.FC<DotMatrixProps> = ({
  ...
  const uniforms = useMemo(() => {
    const colorsArray = colors.length >= 3 ? colors : Array(6).fill(colors[0]);
    ...
    <Shader
      source={`precision mediump float;
        ...
        void main() {
            ...
            ${center.includes("x") ? "st.x -= (mod(u_resolution.x, u_total_size) - u_dot_size) * 0.5;" : ""}
            ${center.includes("y") ? "st.y -= (mod(u_resolution.y, u_total_size) - u_dot_size) * 0.5;" : ""}
